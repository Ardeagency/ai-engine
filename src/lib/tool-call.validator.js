...
export const TOOL_SCHEMAS = {
  ...
  publishDashboardReading: {
    ...
    reading: {
      ...
      properties: {
        ...
        narrative: {
          type: "array",
          description: "OBLIGATORIO. De 1 a 12 bloques tipados, en el orden en que se leen. Tipos: stat_tile {label,value,delta?,direction?,note?} (van PRIMERO, son la fila de KPIs) | insight {title,body,severity:opportunity|warning|threat|neutral,evidence:[evN]} | signal_triangulation {signals:[{observation,source_ref}],so_what} | hypothesis {statement,confidence:alta|media|exploratoria,how_to_verify,evidence:[evN]} | receipt {quote,author_handle?,platform?,engagement?,source_ref} | recommended_move {action,rationale,urgency:hoy|esta_semana|este_mes,evidence:[evN],brief?} | watchlist_item {what,why_watching,check_back?:YYYY-MM-DD} | delta {changed,direction:up|down|new|gone|flat}. SOLO EN COMPETENCIA (scope 'monitoreo'), y son los UNICOS que llenan dos cards que ya existen en pantalla — sin ellos esas cards salen vacias: observacion_perfil {perfil (nombre EXACTO del registrado), titulo (<=60), observacion (que viste y QUE IMPLICA, <=240), rol?:competidor_directo|competidor_indirecto|referente|aliado, severidad?:opportunity|warning|threat|neutral, prioridad?:alta|media|baja, evidence?} llena la card 'Observaciones' — uno por hallazgo, un perfil puede llevar varios | audiencia_competidor {nombre (el GRUPO DE GENTE, no la demografia, <=60), perfil? (quien la pesca), descripcion? (<=200), dolores?:[<=4], deseos?:[<=4], gancho? (el hilo con el que la pesca, <=90), evidence?} llena el carrusel 'Audiencias'.",
          items: { type: "object" },
        },
        evidence: {
          type: "object",
  ...
  publishVera4Card: {
    ...
    scope: {
      ...
      description: "El tab donde vive la card. Cada type pertenece a UNO —publicarlo en otro se rechaza— salvo 'intuicion', que se escribe en monitoreo, tendencias y estrategia, pero UNA DISTINTA POR TAB (mismo metodo, sujeto distinto). Las reglas de los tabs se contradicen entre si (Mi Marca tiene PROHIBIDO nombrar competencia), asi que una card fuera de sitio hace que el tablero diga lo que no debe.",
    },
    periodo: {
      type: "string",
  ...
  exploreSearchDemand:     { term: "string", geo: "string", con_serie: "string" },
  watchSearchTerm:         { term: "string", geo: "string", reason: "string", origen: "string" },
  listWatchedTerms:        { incluir_inactivos: "string" },
  unwatchSearchTerm:       { term: "string", motivo: "string" },
