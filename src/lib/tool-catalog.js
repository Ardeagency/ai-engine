...
const TOOL_EXAMPLES = {
  ...
  getMaterialDeCodigos: { group: "Visibilidad y contenido (CMO)", ex: `[[TOOL:getMaterialDeCodigos|params:{"maxPiezas":12}]]  → tus codigos declarados (color/tipografia/reglas) + piezas reales para MIRARLAS tu y hacer el blink test: tapando el nombre, ¿se reconoce la marca? Anota el resultado con registrarMedicionDeCodigos.` },
  registrarMedicionDeCodigos: { group: "Visibilidad y contenido (CMO)", ex: `[[TOOL:registrarMedicionDeCodigos|params:{"mediciones":[{"tipo":"color","activo":"naranja","piezas_miradas":8,"piezas_con_el":6,"reconocimiento":0.75}]}]]  → anota TU medicion del blink test. La serie es lo unico que dice si la marca MEJORA, no solo como esta hoy.` },
  getSerieDeCodigos: { group: "Visibilidad y contenido (CMO)", ex: `[[TOOL:getSerieDeCodigos]]  → la historia de cada codigo distintivo: mejora, empeora o estable contra tus mediciones anteriores.` },
  getMaterialDeEmpaque: { group: "Visibilidad y contenido (CMO)", ex: `[[TOOL:getMaterialDeEmpaque]]  → productos con sus imagenes de empaque para que las MIRES: ¿comunica la ocasion? ¿se distingue en un anaquel lleno? ¿que promete el frente y que cumple?` },
  ...
  generateImageDirect:{ group: "Generacion de archivos", ex: `[[TOOL:generateImageDirect|params:{"prompt":"Fotografia cenital de un frasco ambar de 250ml sobre marmol blanco veteado, luz lateral suave de ventana, sombra larga hacia la derecha, hojas de eucalipto fuera de foco en la esquina superior izquierda, paleta calida y minimal, etiqueta legible que dice AELIA","aspect_ratio":"4:5","resolution":"2K","output_format":"png"}]]  -> ARRANCA una generacion de imagen REAL (KIE nano-banana-pro). **TU ESCRIBES EL PROMPT FINAL: ya NO hay un modelo intermedio que lo reescriba — tu texto viaja tal cual al generador.** Por eso el prompt debe estar COMPLETO: sujeto, escena, luz, paleta, encuadre, estilo y el texto exacto si la pieza lleva texto. Un prompt de una linea da una imagen generica, y sera culpa del prompt, no del modelo. Campos: prompt (OBLIGATORIO, 10-5000 chars) · aspect_ratio 1:1|2:3|3:2|3:4|4:3|4:5|5:4|9:16|16:9|21:9|auto (default 1:1; feed IG 4:5, historias/Reels 9:16) · resolution 1K|2K|4K (default 2K; 1K para bocetos) · output_format png|jpg (default png) · image_input:["https://..."] opcional, hasta 5 URLs publicas de REFERENCIA (foto real del producto, pieza previa, rostro de un personaje). Si un valor no esta en la lista te lo RECHAZO diciendote las opciones — no lo cambio en silencio. ES ASINCRONA: el tool devuelve rapido con status:"generating" y un task_id REAL; NO trae la imagen. El SISTEMA la ENTREGA en la conversacion cuando el archivo REAL existe (~60-90s). Tu unica respuesta tras llamarlo: UNA linea diciendo que la estas generando y que aparecera aqui en un momento (es honesto: hay un job real). PROHIBIDO: inventar/adivinar un media_url, decir que "ya esta lista", o pegar una URL — si no la entrego yo, no existe. ANTES de generar una imagen normal SIEMPRE PREGUNTA: "quieres que la genere yo directo, o que use un flujo de la biblioteca (getFlows)?".` },
  generateVideoDirect:{ group: "Generacion de archivos", ex: `[[TOOL:generateVideoDirect|params:{"prompt":"Plano medio de una taza humeante sobre una mesa de madera junto a una ventana lluviosa, la camara hace un dolly-in lento, luz gris difusa de tarde, vapor visible a contraluz, ambiente calido y silencioso","aspect_ratio":"9:16","resolution":"720p","duration":8,"generate_audio":true}]]  -> ARRANCA una generacion de video REAL (KIE bytedance/seedance-2-fast). **TU ESCRIBES EL PROMPT FINAL: ya NO hay un modelo intermedio que lo reescriba — tu texto viaja tal cual al generador.** Describe el plano completo: sujeto, accion, MOVIMIENTO DE CAMARA, ambiente, luz, paleta y estilo. Campos: prompt (OBLIGATORIO, 10-5000 chars) · aspect_ratio 21:9|16:9|4:3|1:1|3:4|9:16 (default 16:9; Reels/TikTok 9:16) · resolution 480p|720p|1080p|4k (default 720p) · duration NUMERO entero de 4 a 15 segundos (default 5; cada segundo cuesta) · generate_audio / return_last_frame / web_search true|false. REFERENCIAS opcionales, todas URLs http(s) publicas: first_frame_url y last_frame_url fijan el PRIMER y ULTIMO fotograma (para animar una pieza que ya generaste con generateImageDirect, o definir un recorrido entre dos imagenes) · reference_image_urls:[...] estilo o personaje · reference_video_urls:[...] movimiento y ritmo · reference_audio_urls:[...] voz o musica. Si un valor no esta en la lista te lo RECHAZO diciendote las opciones. ASINCRONA: devuelve rapido con status:"generating" + task_id real; el SISTEMA entrega el video en la conversacion cuando exista (~4 min, mas que una imagen). Tu respuesta: 1 linea de que lo estas generando y que aparecera aqui. PROHIBIDO inventar URL o decir que ya esta. ANTES SIEMPRE PREGUNTA "yo directo o un flujo de la biblioteca?". OJO: KIE pausa modelos de video de su lado por temporadas; si te responde eso, no es tu prompt y no hay nada que reintentar.` },
  ...
  lanzarPredictor: { group: "Inteligencia activa", ex: `[[TOOL:lanzarPredictor|params:{"pregunta":"como reaccionaria el publico a subir el precio 20%","titulo":"Subida de precio","contextoExtra":"el movimiento concreto que se quiere probar","rondas":5}]]  → SIMULA de verdad: arma agentes con personalidad desde el ADN y las audiencias REALES de la marca, los deja debatir por rondas y saca un veredicto. TARDA MINUTOS U HORAS: devuelve un id y ahi termina tu turno — avisale al usuario y consulta despues con getPredictor. NO es el pre-test que razonas en tu cabeza (eso sigue siendo valido y es lo rapido); esto es el motor.` },
  getPredictor: { group: "Inteligencia activa", ex: `[[TOOL:getPredictor|params:{"runId":"<uuid>"}]]  → estado o resultado de una simulacion. Si sigue corriendo te dice en que etapa va. Si fallo, DILO — no inventes un resultado.` },
  listarPredictores: { group: "Inteligencia activa", ex: `[[TOOL:listarPredictores|params:{"limite":10}]]  → historial de simulaciones de la organizacion.` },
};
